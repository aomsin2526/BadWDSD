// stage4j max size = 644

#pragma GCC optimize("align-functions=8")
#pragma GCC optimize("O1")

typedef char int8_t;
typedef unsigned char uint8_t;

typedef short int16_t;
typedef unsigned short uint16_t;

typedef int int32_t;
typedef unsigned int uint32_t;

typedef long int64_t;
typedef unsigned long uint64_t;

typedef uint64_t size_t;

#define ASM(...) asm volatile(__VA_ARGS__)

#define SC()              \
    ASM("sync"); \
    ASM("sc" :"=r"(r3), "=r"(r4), "=r"(r5), "=r"(r6), "=r"(r7):"r"(r11), "r"(r3), "r"(r4), "r"(r5), "r"(r6), "r"(r7):)

#define dead() \
    while (1)  \
    {          \
    }

// in: r3 = laid, r4 = dest, r5 = src
// out: r3 = 0 on success

__attribute__((noreturn)) void stage4j_entry()
{
    register uint64_t r3 asm("r3");

    register uint64_t r4 asm("r4");
    register uint64_t r5 asm("r5");
    register uint64_t r6 asm("r6");
    register uint64_t r7 asm("r7");

    // local use
    register uint64_t r0 asm("r0");
    register uint64_t r8 asm("r8");
    register uint64_t r9 asm("r9");
    register uint64_t r10 asm("r10");

    register uint64_t r11 asm("r11");

    // push stack
    ASM("addi 1, 1, -128");

    // store lr
    ASM("mflr 0");
    ASM("std 0, 0(1)");

    // store laid
    ASM("std 3, 8(1)");

    // store dest
    ASM("std 4, 16(1)");

    // store src
    ASM("std 5, 24(1)");

    {
        // copy src starts

        r11 = 0x10003;

        r4 = 0x6996;
        r5 = 0x20;

        r6 = 0x1111;

        SC();

        // copy src to ra 0xC000000

        {
            // r0 = temp
            // r3 = src
            // r8 = curOffset
            // r9 = left
            // r10 = temp

            // r3 = src
            ASM("ld %0, 24(1)" : "=r"(r3)::); // example: 0xa0008000

            // r8 = 0
            ASM("li %0, 0" : "=r"(r8)::);

            // r9 = 0
            ASM("li %0, 0" : "=r"(r9)::);

            // r10 = file offset
            ASM("ld %0, 0x10(%1)" : "=r"(r10) : "r"(r3) :);

            // r9 += file offset
            ASM("add %0, %1, %2" : "=r"(r9) : "r"(r9), "r"(r10) :);

            // r10 = file size
            ASM("ld %0, 0x18(%1)" : "=r"(r10) : "r"(r3) :);

            // r9 += file size
            ASM("add %0, %1, %2" : "=r"(r9) : "r"(r9), "r"(r10) :);

            //
            ASM("src_copy_loop:");

            r11 = 0x10003;

            r4 = 0x6996;

            r6 = 0xC000000;
            r6 += r8;

            r10 = r3;
            r10 += r8;

            if (r9 < 8)
            {
                r5 = 0x8; // poke8
                ASM("lbz %0, 0(%1)" : "=r"(r7) : "r"(r10) :);
                r10 = 1;
            }
            else
            {
                r5 = 0x2; // poke64
                ASM("ld %0, 0(%1)" : "=r"(r7) : "r"(r10) :);
                r10 = 8;
            }

            // store register to stack
            ASM("std %0, 32(1)" ::"r"(r0) :);
            ASM("std %0, 40(1)" ::"r"(r8) :);
            ASM("std %0, 48(1)" ::"r"(r9) :);
            ASM("std %0, 56(1)" ::"r"(r10) :);
            ASM("std %0, 64(1)" ::"r"(r3) :);

            SC();

            // restore register from stack
            ASM("ld %0, 64(1)" : "=r"(r3)::);
            ASM("ld %0, 56(1)" : "=r"(r10)::);
            ASM("ld %0, 48(1)" : "=r"(r9)::);
            ASM("ld %0, 40(1)" : "=r"(r8)::);
            ASM("ld %0, 32(1)" : "=r"(r0)::);

            // curOffset += r10
            ASM("add %0, %1, %2" : "=r"(r8) : "r"(r8), "r"(r10) :);

            // left -= r10
            ASM("sub %0, %1, %2" : "=r"(r9) : "r"(r9), "r"(r10) :);

            // break if left == 0
            ASM("li %0, 0" : "=r"(r0)::);
            ASM("cmp 0, 0, %0, %1" ::"r"(r9), "r"(r0) :);
            ASM("beq src_copy_done");

            ASM("b src_copy_loop");
        }

        ASM("src_copy_done:");

        // copy src done

        r11 = 0x10003;

        r4 = 0x6996;
        r5 = 0x20;

        r6 = 0x2222;

        SC();
    }

    // 4C4F4144 4D455858 58584C4F 41444D45
    // write loadme into dest

    {
        // r10 = dest
        ASM("ld %0, 16(1)" : "=r"(r10)::);

        r3 = 0x4C4F4144;
        ASM("stw %0, 0(%1)" ::"r"(r3), "r"(r10) :);
        r10 += 4;

        r3 = 0x4D455858;
        ASM("stw %0, 0(%1)" ::"r"(r3), "r"(r10) :);
        r10 += 4;

        r3 = 0x58584C4F;
        ASM("stw %0, 0(%1)" ::"r"(r3), "r"(r10) :);
        r10 += 4;

        r3 = 0x41444D45;
        ASM("stw %0, 0(%1)" ::"r"(r3), "r"(r10) :);
        r10 += 4;
    }

    // Call stage4

    r11 = 0x10003;

    r4 = 0x6996;
    r5 = 0x21;

    SC();

    // everything done

    r11 = 0x10003;

    r4 = 0x6996;
    r5 = 0x20;

    r6 = 0x3333;

    SC();

    //

    // restore lr
    ASM("ld 0, 0(1)");
    ASM("mtlr 0");

    // pop stack
    ASM("addi 1, 1, 128");

    // set r3
    ASM("li 3, 0");

    // return
    ASM("blr");

    __builtin_unreachable();
}