...
void Stagex_spu_job_stage2_qcfw_jig(const struct Stagex_spu_job_stage2_context_s *job_context)
{
    ...
    uint8_t found_force_sc_remarry = 0;
    uint8_t found_force_sc_remarry_2 = 0;

    uint8_t found_skip_sc_format_error = 0;
    uint8_t found_sc_check_product_mode_capability = 0;
    ...
    for (uint64_t tmpBuf_CurEaAddr = 0; tmpBuf_CurEaAddr < (8 * 1024 * 1024); tmpBuf_CurEaAddr += tmpBufSize)
    {
        ...
        for (uint32_t i = 0; i < tmpBufSize; i += 4)
        {
            ...
            // This patch will always force remarry second time (SC_INIT_FLAG_NEED_PERSONALIZE?) even on working console, but will only work when in FSM.

            if (!found_force_sc_remarry)
            {
                // puts("Patching found_force_sc_remarry...\n");

                __attribute__((aligned(16))) static const uint8_t searchData[] = {0xF8, 0x21, 0xFF, 0x21, 0x7C, 0x08, 0x02, 0xA6, 0xFB, 0xA1, 0x00, 0xC8, 0x3B, 0xA1, 0x00, 0x70, 0xFB, 0x61, 0x00, 0xB8, 0x7C, 0x7B, 0x1B, 0x78, 0x7F, 0xA3, 0xEB, 0x78, 0xFB, 0x81, 0x00, 0xC0, 0x7C, 0x9C, 0x23, 0x78, 0xF8, 0x01, 0x00, 0xF0, 0x48, 0x00, 0x02, 0xD5, 0x7F, 0xA3, 0xEB, 0x78, 0x48, 0x01, 0x3D, 0x25, 0x80, 0x01, 0x00, 0xA4, 0x81, 0x21, 0x00, 0xA8};
                __attribute__((aligned(16))) static const uint8_t replaceData[] = {0xF8, 0x21, 0xFF, 0x21, 0x7C, 0x08, 0x02, 0xA6, 0xFB, 0xA1, 0x00, 0xC8, 0x3B, 0xA1, 0x00, 0x70, 0xFB, 0x61, 0x00, 0xB8, 0x7C, 0x7B, 0x1B, 0x78, 0x7F, 0xA3, 0xEB, 0x78, 0xFB, 0x81, 0x00, 0xC0, 0x7C, 0x9C, 0x23, 0x78, 0xF8, 0x01, 0x00, 0xF0, 0x48, 0x00, 0x02, 0xD5, 0x7F, 0xA3, 0xEB, 0x78, 0x48, 0x01, 0x3D, 0x25, 0x80, 0x01, 0x00, 0xA4, 0x39, 0x20, 0x00, 0x00};

                if (!memcmp32(&tmpBuf[i], searchData, sizeof(searchData)))
                {
                    DMAWrite(replaceData, curEaAddr, sizeof(replaceData));
                    found_force_sc_remarry = 1;
                }
            }

            if (!found_force_sc_remarry_2)
            {
                // puts("Patching found_force_sc_remarry_2...\n");

                __attribute__((aligned(16))) static const uint8_t searchData[] = {0x7F, 0xA3, 0xEB, 0x78, 0x83, 0xA1, 0x00, 0x88, 0x90, 0x1B, 0x00, 0x00, 0x91, 0x3C, 0x00, 0x00, 0x48, 0x00, 0x07, 0x91, 0xE8, 0x01, 0x00, 0xF0, 0xEB, 0x61, 0x00, 0xB8, 0x7B, 0xA3, 0x00, 0x20};
                __attribute__((aligned(16))) static const uint8_t replaceData[] = {0x7F, 0xA3, 0xEB, 0x78, 0x83, 0xA1, 0x00, 0x88, 0x90, 0x1B, 0x00, 0x00, 0x91, 0x3C, 0x00, 0x00, 0x48, 0x00, 0x07, 0x91, 0xE8, 0x01, 0x00, 0xF0, 0xEB, 0x61, 0x00, 0xB8, 0x38, 0x60, 0x00, 0x00};

                if (!memcmp32(&tmpBuf[i], searchData, sizeof(searchData)))
                {
                    DMAWrite(replaceData, curEaAddr, sizeof(replaceData));
                    found_force_sc_remarry_2 = 1;
                }
            }

            if (!found_skip_sc_format_error)
            {
                // puts("Patching found_skip_sc_format_error...\n");

                __attribute__((aligned(16))) static const uint8_t searchData[] = {0xFB, 0xE1, 0x00, 0xD8, 0xF8, 0x01, 0x00, 0xF0, 0x48, 0x00, 0x62, 0x65, 0x2F, 0xA3, 0x00, 0x00, 0x40, 0x9E, 0x00, 0x0C, 0xE8, 0x62, 0x80, 0x38, 0x48, 0x00, 0x01, 0xD8, 0x48, 0x00, 0x62, 0x51, 0x48, 0x00, 0x75, 0x15};
                __attribute__((aligned(16))) static const uint8_t replaceData[] = {0xFB, 0xE1, 0x00, 0xD8, 0xF8, 0x01, 0x00, 0xF0, 0x48, 0x00, 0x62, 0x65, 0x2F, 0xA3, 0x00, 0x00, 0x40, 0x9E, 0x00, 0x0C, 0xE8, 0x62, 0x80, 0x38, 0x48, 0x00, 0x01, 0xD8, 0x48, 0x00, 0x62, 0x51, 0x38, 0x60, 0x00, 0x00};

                if (!memcmp32(&tmpBuf[i], searchData, sizeof(searchData)))
                {
                    DMAWrite(replaceData, curEaAddr, sizeof(replaceData));
                    found_skip_sc_format_error = 1;
                }
            }

            if (!found_sc_check_product_mode_capability)
            {
                // puts("Patching found_sc_check_product_mode_capability...\n");

                __attribute__((aligned(16))) static const uint8_t searchData[] = {0x4B, 0xFE, 0x59, 0xA9, 0x38, 0x00, 0x00, 0x00, 0x48, 0x00, 0x00, 0x18, 0x7C, 0x03, 0x03, 0x78, 0xE8, 0xA2, 0x99, 0x78, 0x38, 0x80, 0x00, 0x02, 0x4B, 0xFE, 0x59, 0x91, 0x38, 0x00, 0x00, 0x05, 0x39, 0x20, 0x00, 0x05, 0x7C, 0x03, 0x48, 0x38};
                __attribute__((aligned(16))) static const uint8_t replaceData[] = {0x4B, 0xFE, 0x59, 0xA9, 0x38, 0x00, 0x00, 0x00, 0x48, 0x00, 0x00, 0x18, 0x7C, 0x03, 0x03, 0x78, 0xE8, 0xA2, 0x99, 0x78, 0x38, 0x80, 0x00, 0x02, 0x4B, 0xFE, 0x59, 0x91, 0x38, 0x00, 0x00, 0x05, 0x39, 0x20, 0x00, 0x05, 0x38, 0x60, 0x00, 0x00};

                if (!memcmp32(&tmpBuf[i], searchData, sizeof(searchData)))
                {
                    DMAWrite(replaceData, curEaAddr, sizeof(replaceData));
                    found_sc_check_product_mode_capability = 1;
                }
            }
        }
    }
...
void Stagex_spu_job_stage2(const struct Stagex_spu_job_stage2_context_s *job_context)
{
    if (job_context->is_qcfw_jig)
        ...
        Stagex_spu_job_stage2_generic(job_context);
}
